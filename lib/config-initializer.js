...
const moduleFunction = function(args = {}) {
	...
	const getElement = ((config, configName, configFileName, commandLineParameters) => item => {
		let configItem;
		...
		if (typeof(item)=='string'){
			...
			configItem = config.qtGetSurePath(item.configElementName, {
				warning: `no config was found for configElementName:'${item.configElementName}' in ${configName}/${configFileName}`
			});
		}
			...
			configItem = config.qtGetSurePath(moduleName, {
				warning: `no config was found for filename:'${item.configString}' in ${configName}/${configFileName}`
			});
		}
		else {
			console.error(
...
console.trace()
		...
		return subStringPath=>{
			...
			if (subStringPath=='.'){
				return {...configItem, commandLineParameters, _global: config.global, _meta,_log: log  };
			}
			...
			return {...configElement, commandLineParameters, _global: config.global, _meta ,_log: log  };
	...
	const makeRetriever=(getElement=>(retriever, codeRoot)=>{
	
		const retrieverConfig = getElement(retriever)('.');
		return require(path.join(
			codeRoot,
			retrieverConfig.modulePath
		))({retrieverConfig});
