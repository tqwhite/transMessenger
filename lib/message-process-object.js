...
const moduleFunction = function({ messages } = {}) {
	...
	const validMessages = messages => messages.length && messages.filter(item=>(item.phoneNumbers && item.message))?true:false;
	console.error(`${path.basename(__filename)} SAYS: validate phone number/type pairs "item.phoneNumbers.filter(item=>(item.type && item.number))" someday`);
